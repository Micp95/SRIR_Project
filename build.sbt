...
val commonSettings = Seq(
  moduleName := "compileproject-" + moduleName.value,
)
...
lazy val root = project.in(file("."))
  ...
  .settings(
    ...
    Compile / mainClass := Some("srir.backend.Launcher")
  )
...
lazy val frontend = project.in(file("frontend"))
  ...
  .settings(
    ...
    Compile / mainClass := Some("srir.frontend.JSLauncher"),
    ...
    compileCss := Def.taskDyn {
      ...
      dir.mkdirs()
      (backend / Compile / runMain).toTask(s" srir.backend.css.CssRenderer $path false")
    }.value,
